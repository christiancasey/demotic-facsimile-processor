...
for i = 1:nGlyphSets
	...
	disp(sprintf('Processing image part: %s', vGlyphKeys{i}));
	...
	% Not needed for classifier layer -- already connected and separated
...
% 		plot(vGlyphPoly(1:end-1,1),vGlyphPoly(1:end-1,2),'-k','LineWidth',3);
% 		plot(iStartX,iStartY,'*w','MarkerSize',10);
% 		hold off;
...
% Sort glyphs by page, line, word, (morpheme)
v = cell2mat(vGlyphs(:,2:5));
[~,i] = sortrows(v,1:size(v,2));
vGlyphs = vGlyphs(i,:);
